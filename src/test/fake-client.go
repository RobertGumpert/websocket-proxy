...
func (this *FakeClientWrapper) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case "/ws":
		...
		log.Println(this.Unit.LocalAddr)
		go this.send()
		go this.receive()
	...
	if this.ID == 0 {
		...
		for range timer.C {
			err := this.Unit.CreateMessageAndSend(
				fmt.Sprintf("Message by client [%d], address [%s]", this.ID, this.URL),
			)
			if err != nil {
				log.Fatal(err)
			}
...
func (this *FakeClientWrapper) receiveErrors() {
	for _ = range this.closeChannel {
		log.Println(fmt.Sprintf("---> Client [%d] close the connection, address [%s]", this.ID, this.URL))
	}
}
